<?php
/* @var $this UniversidadesController */
/* @var $model Universidades */
/* @var $form CActiveForm */
?>

<div class="form">

<?php $form=$this->beginWidget('CActiveForm', array(
	'id'=>'universidades-form',
	'enableAjaxValidation'=>false,
)); ?>

	<p class="note">Fields with <span class="required">*</span> are required.</p>

	<?php echo $form->errorSummary($model); ?>

	<div class="row">
		<?php echo $form->labelEx($model,'nom_univ'); ?>
		<?php echo $form->textField($model,'nom_univ',array('size'=>60,'maxlength'=>100)); ?>
		<?php echo $form->error($model,'nom_univ'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'univ_web'); ?>
		<?php echo $form->textField($model,'univ_web',array('size'=>60,'maxlength'=>100)); ?>
		<?php echo $form->error($model,'univ_web'); ?>
	</div>

	<div class="row buttons">
		<?php echo CHtml::submitButton($model->isNewRecord ? 'Create' : 'Save'); ?>
	</div>

<?php $this->endWidget(); ?>

</div><!-- form -->